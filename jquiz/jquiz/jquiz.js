...
// Sounds to play
audio_applause = new Audio('/jquiz/jquiz/applause_normal.mp3');
...
function jquiz_next_question(quiz_id) {
    ...
    // The following code is required because checking if quiz_id is in
    // the keys array always gives false, even when it should be true
    id_in_results = false;
    ids = Object.keys(quiz_results);
    ids.forEach(k => {
        if (quiz_id == k)
            id_in_results = true;
    });
    if (id_in_results) {
        ...
        if (quiz_results[quiz_id]['completed']) {
            ...
            console.log("JQuiz: Quiz was completed. Restarting.");
            // Don't restart - display the results etc. instead.
            ...
            console.log("JQuiz: Quiz is incomplete.");
        }
    } else {
        // Initialise a new quiz
    ...
    // Might need to check that the quiz has loaded first, and do
    ...
    question.answers.forEach((answer, index) => {
        answer_id = (Math.random() * 10**20).toString(); // Not guaranteed unique
        answer_code.push(`
    ...
    if (quiz_results[quiz_id]['q_index'] >=
        quiz_results[quiz_id]['num_questions']) {
            ...
            // Get rid of "next question" button or something.
    }
